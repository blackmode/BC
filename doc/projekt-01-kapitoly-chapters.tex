...
U perspektivní matice je situace obdobná jako modelové matice. V \texttt{\setupProgram} se pouze vytvoří a nahraje do shaderu, dale se v programu již nemění. Naproti tomu zobrazovací matice, která plni roli pozorovatele ve scéně resp. Roli kamery, se pokaždé interakci se vstupním zařízením mezi a je nutné její data nahrát do shaderu znova. 


\subsection{Shadery}
Veškeré operace se shadery v zásadě probíhají ve funkci \texttt{\setupProgram}. Samotný kód vertex a fragment shaderu se předává jako textová řetězec, tudíž jej můžeme uložit do proměnné. V našem konkrétním případě  ale pro umístění shaderu využíváme kód mezi HTML značkami \texttt{<script></script>}. Důvodem je ten, že na zpracování programu to nemá vůbec žádný vliv  a  kód je daleko lépe editovatelný. Každý shader ma svoje specifické označení, pro fragment shader je nastaven v tágu \texttt{<script>} atribut type na \texttt{x-shader/x-fragment}, ve vertex shaderu potom \texttt{x-shader/x-vertex}.

Výpočty transformačních matic se počítají v hlavním programu, ale přímé operace s nimi až v shaderech. Všechny matice se zde vynásobí aktuálním bodem geometrie, čímž dosáhneme požadované projekce bodu v prostoru. K nastavení konečného bodu v prostoru s vypočtenou projekcí slouží gl\_Position který je roven $PMatrix \cdot VMatrix \cdot MMatrix \cdot vec4(pos, 1.0)$
