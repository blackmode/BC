...
Cílem modelové matice je tedy transformovat body modelu ze středu souřadného systému, vůču kterému byly body geometrie modelu napočítány, do zobrazitelného prostoru, se kterým budeme dále pracovat. Začneme s matici pro změnu měřítka modelu:
...
Takového efektu docílíme tak, že složku o kterou se chceme posunout vynásobíme opačnou hodnotou posunu.
...
\begin{description}
	\item[fieldOfViewInRadians] description
			\item[aspectRation] description
		\item[near] description
		\item[far] description
\end{description}

$ f = \dfrac{1}{\tan\biggr(\dfrac{ field\_of\_view }{2}\biggr)} $
$$
P =
\begin{pmatrix} 
f/aspect & 0 & 0 & 0\\
0 & f & 0 & 0\\ 
0 & 0 & (near+far) \cdot (1/(near-far)) & -1\\ 
0 & 0 & (near \cdot far \cdot (1/(near-far)) \cdot 2) & 1\\ 
\end{pmatrix}
$$
