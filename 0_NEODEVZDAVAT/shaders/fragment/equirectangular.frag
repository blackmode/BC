...
varying vec2 texCoordVarying; //PUVODNÍ: in vec2 texCoordVarying;
//PUVODNÍ: out vec4 fragColor; klasicky je ale gl_fragColor jako OUT

uniform vec2 resolution;
uniform sampler2DRect source;// vypada to, ze tohle je novejsi verze tohohle: 'uniform sampler2D source', tohle je asi textura
uniform float blendingRatio; // 1.0=no alpha blending
uniform CameraParams cam[2]; // nejaky data z kamery ohledne natoceni apod., dost se to pouziva!!!
...
/*
    PARAMETRY:
        ...
        float translate = 0.0;
        // now resolution.x is 1920, resolution.y is 960, no proste tam bude hodnota: [1920,960] vektor od vou prvcich, thats all
        const ofVec2f resolution = ofVec2f(1920, 960);

    SPRAVNE NASTAVENI PROMENNE cam v JS:
